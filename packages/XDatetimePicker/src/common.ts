export function getFullDatetime(datetime) {
  return String(datetime).length < 2 ? `0${datetime}` : datetime;
}

export function resetFullDatetime(datetime) {
  let curDatetime = '';
  if (datetime) {
...
export function getCorrectDatetime(year, month = '', day = '', hour = '', minute = '', second = '', millisecond = '') {
